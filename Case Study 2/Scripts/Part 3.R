...
boxplot(nonZeroKWHEnergy$kWh, nonZeroKWHEnergy$Peakhour)
...
#Make all the zero values for kwh in zeroKWHEnergy as NA
zeroKWHEnergy$kWh[which(zeroKWHEnergy$kWh == 0)] <- NA
#View(summary(zeroKWHEnergy))

#bind the nonzero dataset with zero kwh energy dataset
bind_energyData <- bind_rows(nonZeroKWHEnergy,zeroKWHEnergy)

#sort the dataset based on dataset and then hour
sort_bind_energy <- bind_energyData[order(as.Date(bind_energyData$Date), bind_energyData$hour),]

#Replacing the kwh values using package zoo
#install.packages("zoo")
library(zoo)
#sort_bind_energy$kWh<-na.aggregate(bind_energyData$kWh, by = bind_energyData$kWh, FUN = mean,na.rm = TRUE)
#sort_bind_energy$kWh <-na.aggregate(bind_energyData$kWh, FUN = mean,na.rm = TRUE)   rsq = 0.1221
#sort_bind_energy$kWh <-na.approx(sort_bind_energy$kWh, na.rm = FALSE, rule = 2)  rsq=0.2246
sort_bind_energy$kWh <-na.locf(sort_bind_energy$kWh) #rsq = 0.433

#summary of the whole dataset
#View(summary(energy))
#View(summary(zeroKWHEnergy))
#View(summary(nonZeroKWHEnergy))
#View(summary(sort_bind_energy))

write.csv(energy, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 3/energy.csv")
write.csv(sort_bind_energy, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 3/sort_bind_energy.csv")
write.csv(zeroKWHEnergy, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 3/zeroKWHEnergy.csv")
write.csv(nonZeroKWHEnergy, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 3/nonZeroKWHEnergy.csv")
...
tempData <- getWeatherForDate("KBOS", start_date=min(sort_bind_energy$Date),
                              end_date = max(sort_bind_energy$Date),
                              opt_detailed=T,
...
#write to csv file
write.csv(aggdata, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 3/Temperature.csv")

# merging the dataset
library(dplyr)
consolidate <- left_join(sort_bind_energy, aggdata, by = c("Date"="Date", "hour"= "Hour"))
...
#Writing it to csv
write.csv(consolidate, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 3/Hourly_filled_data.csv")
write.csv(consolidate, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Hourly_filled_data.csv")
...
lm.fit <- lm(kWh ~ Temp + Peakhour + hour + Weekday  + month + day + `Day of Week` + Date, data = train)
...
#write.csv(pred, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 3/Prediction.csv")
...
tidy_lmfit[,1:2]
write.csv(tidy_lmfit[,1:2], file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 3/RegressionOutputs.csv")
...
write.csv(performance, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 3/PerformanceMetrics.csv")