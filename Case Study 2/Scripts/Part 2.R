...
write.csv(energy, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 2/energy.csv")
write.csv(zeroKWHEnergy, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 2/zeroKWHEnergy.csv")
write.csv(nonZeroKWHEnergy, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 2/nonZeroKWHEnergy.csv")

#75% of the sample size
smp_size <- floor(0.75 * nrow(nonZeroKWHEnergy))

#Set the seed to make your partition reproductible
set.seed(1)
train_ind <- sample(seq_len(nrow(nonZeroKWHEnergy)), size = smp_size)

#Split the data into training and testing
train <- nonZeroKWHEnergy[train_ind, ]
test <- nonZeroKWHEnergy[-train_ind, ]

##Linear Model
lm.fit = lm(kWh ~ ., data = train)

#Modified Linear Model
lm.fit <- lm(kWh ~ Peakhour + hour + Weekday + month + day + Date, data = train) #Removed the Date
...
#Forecasting kWH value of zerokwhEnergyDataset without Temp
library(gdata)
forecastInput <- zeroKWHEnergy;

#dropping kWh
drops <- c("kWh");
forecastInput <- forecastInput[ , !(names(forecastInput) %in% drops)]

#Preparing the dataset for forecasting
#dataset having zero kWh
#View(forecastInput)

#predict the power usage in KWH for each Hour.
library(forecast)
pred = predict(lm.fit, forecastInput)
kWh <- pred
#View(pred)

forecastOutput <- cbind(forecastInput,kWh)
#View(summary(forecastOutput))

#write to csv file
write.csv(forecastOutput, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 2/zeroKWHEnergy.csv")

#rearraging kWh in zerokWh dataset
col_idx <- grep("kWh", names(nonZeroKWHEnergy))
nonZeroKWHEnergy <- nonZeroKWHEnergy[, c((1:ncol(zeroKWHEnergy))[-col_idx], col_idx)]
names(nonZeroKWHEnergy)
names(forecastOutput)

#Merging zero and non-zero kWh dataset
merge <- rbind(nonZeroKWHEnergy, forecastOutput)

#summary of merged dataset
View(merge)
View(summary(merge))

#write hourly data file to csv
write.csv(consolidate, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 2/Hourly_filled_data.csv")
...
tempData <- getWeatherForDate("KBOS", start_date=min(merge$Date),
                              end_date = max(merge$Date),
                              opt_detailed=T,
...
#write to csv file
write.csv(aggdata, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 2/Temperature.csv")

# merging the energy dataset with temperature
library(dplyr)
consolidate <- left_join(merge, aggdata, by = c("Date"="Date", "hour"= "Hour"))
...
#Modified Linear Model
lm.fit <- lm(kWh ~ Temp + Peakhour + hour + Weekday  + month + Date, data = train)
...
#write.csv(pred, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 2/Prediction.csv")
...
tidy_lmfit[,1:2]
write.csv(tidy_lmfit[,1:2], file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 2/RegressionOutputs.csv")
...
write.csv(performance, file="/Users/hpanjwani/R Workspace/Assignment 2/Output/Approach 2/PerformanceMetrics.csv")